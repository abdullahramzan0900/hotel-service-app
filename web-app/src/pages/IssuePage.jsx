...
export default function IssuePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const messageError = values.message.trim().length < 3 ? 'Please describe the issue.' : null;
    ...
    setServerError('');
    try {
      await submitIssue(token, { ...values, priority });
      navigate(`/r/${token}/confirmation`, {
        state: { title: 'Issue Reported', message: 'Thank you for letting us know. Our team is on it.' }
      });
    } catch (err) {
  ...
  return (
    <div className="page form-page fade-in">
      <div className="form-header">
        ...
        <p className="muted">Tell us what's wrong so we can fix it quickly.</p>
      </div>
      ...
      <div className="page-content">
        <form onSubmit={handleSubmit}>
          ...
          <div className="field">
            ...
            <textarea
              placeholder="e.g. The air conditioning isn't working"
          ...
          <button className="btn btn-primary" type="submit" disabled={submitting}>
            {submitting ? <span className="spinner" /> : 'Report Issue'}
          </button>
        </form>
      </div>
    </div>
